...
def classify_data(data):
    ...
    unique_classes, count_unique_classes = np.unique(label_column, return_counts=True)  # what are the elements and how many appearances
...
def update_data_column(partial_data, data, split_column):
    if partial_data.size == 0:
        ...
        partial_data = np.delete(partial_data, np.s_[split_column], axis=1)    # drop the used column from the data to the next decision
    return partial_data
...
def is_busy(row_input):
    np_input = np.array(row_input)
    np_busy = np.insert(np_input, 1, "busy")
    row_values = pd.Series(np_busy)
    if row_values is not None:
        best_tree, label_row = tree_error(3, True)
